...
This document defines spec version **0.1**. Versioning rules are in [§7](#7-versioning).
...
|---|---|---|---|
| `version` | Yes | string | Spec version this schema conforms to. Currently `0.1`. |
| `events` | Yes | object | Map of event name → event entry. Event names MUST match `^[a-z][a-z0-9_]*$` (snake_case ASCII). |

Top-level fields prefixed with `x-` are reserved for tool-specific extensions and MUST be ignored by implementations that don't recognize them. See [§8](#8-extension-fields).
