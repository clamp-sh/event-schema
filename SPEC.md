...
This document defines spec version **0.3**. Versioning rules are in [§10](#10-versioning).
...
experiments:    # optional, added in 0.2
  ...
metrics:        # optional, added in 0.3
  ...
```

| Field | Required | Type | Description |
|---|---|---|---|
| `version` | Yes | string | Spec version this schema conforms to. Currently `0.3`. |
...
| `metrics` | No | object | Map of metric name → metric entry. See [§9](#9-metrics). Added in 0.3. |

Top-level fields prefixed with `x-` are reserved for tool-specific extensions and MUST be ignored by implementations that don't recognize them. See [§11](#11-extension-fields).
...
Optional. Declares named metrics derived from the event stream — the *questions* you ask of your analytics, declared once so every consumer (codegen, agents, dashboards, BI tools) shares the same definition. Two metric types in 0.3: `count` and `ratio`.

### count

A metric counted from a single event, optionally filtered by a property value.

```yaml
metrics:
  daily_signups:
    type: count
    intent: New signups per day. Primary top-of-funnel metric.
    event: signup_completed
    grain: day

  weekly_pro_signups:
    type: count
    intent: Pro-tier signups per week.
    event: signup_completed
    grain: week
    filter:
      property: plan
...
|---|---|---|---|
| `type` | Yes | `"count"` | — |
| `event` | Yes | string | Event name to count. MUST exist in the `events` section. |
| `grain` | Yes | string | One of `day`, `week`, `month`. Time bucket the count is computed against. |
| `intent` | Recommended | string | One sentence on what this metric is *for*. Same role as event `intent`. |
| `filter` | No | object | Restrict the count to events where `property == value`. Both fields required if filter is present. |

### ratio

A metric defined as the ratio of two counts over the same period.
...
metrics:
  weekly_signup_to_paid_cvr:
    type: ratio
    intent: |
      % of weekly signups that converted to a paid plan in the same week.
      Read as a board-level lever; not a strict cohort cohort retention metric.
    grain: week
    numerator:
...
|---|---|---|---|
| `type` | Yes | `"ratio"` | — |
| `numerator` | Yes | object | Same shape as a `count`-metric body: `event` + optional `filter`. |
| `denominator` | Yes | object | Same shape. |
| `grain` | Yes | string | `day` / `week` / `month`. Both counts use the same grain. |
| `intent` | Recommended | string | One sentence. |

The 0.3 ratio is *not* cohort-windowed — it's `count(numerator) / count(denominator)` over the same period bucket. Cohort-windowed metrics ("of users who fired X within 30 days, how many fired Y?") are deliberately not in 0.3; they require user-journey logic that's a heavier addition. May be added in a later version as a third metric type.
